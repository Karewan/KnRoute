...
PHP);
	...
	if (($initialCache[4] ?? null) !== 4) throw new RuntimeException('Cache format version is missing from cache.');
	...
	(new Router())->registerRoutesFromControllers($controllersPath, $cacheFile, true);
	if (((require $cacheFile)[4] ?? null) !== 4) throw new RuntimeException('An incompatible cache format was not regenerated.');
